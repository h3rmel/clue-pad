...
[docs/SPEC-v2.md](docs/SPEC-v2.md) specs the **post-MVP** features (i18n pt-BR/en-US/es — UI chrome only, item names stay pt-BR; dark mode following system + toggle; placeholder asset refresh; UI/UX). New `localStorage` keys it introduces: `clue-pad:locale:v1`, `clue-pad:theme:v1`. **Phases 1 (i18n), 2 (dark mode), 3 (new state + color rework), and 5 (header/mobile polish) are implemented** — see the notes under Core architecture; Phase 4 (placeholder refresh) is the remaining track. Phase 5 consolidated ThemeSelect + LanguageSelect into a single `SettingsMenu` (DropdownMenu with two radio groups), right-aligned the VersionSelect on mobile, and tightened card transitions. [docs/ROADMAP-v2.md](docs/ROADMAP-v2.md) tracks phase progress.
