...
### Placeholder generation (SPEC-v2 §3, implemented)
The 27 generic placeholders in `public/games/estrela-2020/` are produced by `scripts/generate-placeholders.mjs` (run via `pnpm placeholders`). It holds a `slug → { bg, icon }` manifest, reads Lucide icon node data from the installed `lucide-react` package at build time, composes an SVG (colored background + centered icon, dark/light stroke auto-picked by background luminance), and rasterizes to 512×512 PNG via `@resvg/resvg-js` (devDependency). Suspects use their physical-board pawn color; weapons share a steel background, places a stone one, each differentiated by an item-referencing icon; "Dona Branca" gets an off-white background + dark border so it stays visible in the light theme. **This is build/design-time tooling only — it does not enter the runtime data model, and the runtime code still does not distinguish placeholder from licensed art.** Editing colors/icons = edit the manifest and re-run.
...
[docs/SPEC-v2.md](docs/SPEC-v2.md) specs the **post-MVP** features (i18n pt-BR/en-US/es — UI chrome only, item names stay pt-BR; dark mode following system + toggle; placeholder asset refresh; UI/UX). New `localStorage` keys it introduces: `clue-pad:locale:v1`, `clue-pad:theme:v1`. **Phases 1 (i18n), 2 (dark mode), 3 (new state + color rework), 4 (placeholder refresh), and 5 (header/mobile polish) are implemented** — see the notes under Core architecture. Phase 5 consolidated theme + language controls into a single `SettingsMenu` (shadcn `Sheet` with theme/language option buttons; on mobile the VersionSelect and ResetButton also live inside it, while sm+ keeps them in the header), and tightened card transitions. [docs/ROADMAP-v2.md](docs/ROADMAP-v2.md) tracks phase progress.
